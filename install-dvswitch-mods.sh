...
readonly MMDVM_DIR="/var/lib/mmdvm" BACKUP_ROOT="/var/backups/dvswitch-mods" MMDVM_SERVICE="mmdvm_bridge.service"
readonly P25_JSON="$MMDVM_DIR/P25Hosts.json" NXDN_JSON="$MMDVM_DIR/NXDNHosts.json"
WORK_DIR=""; INSTALL_ACTIVE=0; SERVICE_RESTART_NEEDED=0
die(){ printf 'ERROR: %s\n' "$1" >&2; exit 1; }
...
preflight(){ require_root; for c in bash chmod chown cmp cp curl date file grep install mktemp mv php python3 sha256sum stat systemctl wc; do require_command "$c"; done; check_os; for f in "$MMDVM_PATCHER" "$PHP_PATCHER" "$TRANSACTION_LIBRARY" "$MMDVM_BINARY" "$DVSWITCH_SCRIPT" "$DASHBOARD_FUNCTIONS" "$DASHBOARD_STATUS"; do check_regular_file "$f"; done; [[ -d "$MMDVM_DIR" && ! -L "$MMDVM_DIR" ]] || die "Invalid MMDVM directory."; file -b "$MMDVM_BINARY" | grep -q ELF || die "MMDVM_Bridge is not an ELF executable."; systemctl cat "$MMDVM_SERVICE" >/dev/null; php -l "$DASHBOARD_FUNCTIONS" >/dev/null; php -l "$DASHBOARD_STATUS" >/dev/null; bash -n "$DVSWITCH_SCRIPT"; python3 -m py_compile "$MMDVM_PATCHER" "$PHP_PATCHER"; }
...
validate_mmdvm_candidate(){ file -b "$1" | grep -q ELF; python3 "$MMDVM_PATCHER" --binary "$1"; }
prepare_candidates(){ WORK_DIR=$(mktemp -d /tmp/dvswitch-mods.XXXXXX); cp -- "$MMDVM_BINARY" "$WORK_DIR/MMDVM_Bridge"; python3 "$MMDVM_PATCHER" --binary "$WORK_DIR/MMDVM_Bridge"; validate_mmdvm_candidate "$WORK_DIR/MMDVM_Bridge"; cp -- "$DASHBOARD_FUNCTIONS" "$WORK_DIR/functions"; cp -- "$DASHBOARD_STATUS" "$WORK_DIR/status"; cp -- "$DVSWITCH_SCRIPT" "$WORK_DIR/dvswitch"; python3 "$PHP_PATCHER" --functions "$WORK_DIR/functions" --status "$WORK_DIR/status" --dvswitch "$WORK_DIR/dvswitch"; php -l "$WORK_DIR/functions" >/dev/null; php -l "$WORK_DIR/status" >/dev/null; bash -n "$WORK_DIR/dvswitch"; download_json P25 "$WORK_DIR/P25Hosts.json"; download_json NXDN "$WORK_DIR/NXDNHosts.json"; }
show_changes(){ for pair in "$MMDVM_BINARY:$WORK_DIR/MMDVM_Bridge" "$DASHBOARD_FUNCTIONS:$WORK_DIR/functions" "$DASHBOARD_STATUS:$WORK_DIR/status" "$DVSWITCH_SCRIPT:$WORK_DIR/dvswitch"; do live=${pair%%:*}; candidate=${pair#*:}; if cmp -s "$live" "$candidate"; then printf 'UNCHANGED: %s\n' "$live"; else printf 'CHANGE READY: %s\n' "$live"; fi; done; printf 'JSON READY: %s and %s\n' "$P25_JSON" "$NXDN_JSON"; }
...
run_dry_run(){ preflight; prepare_candidates; show_changes; printf 'PASS: all ordered candidates validated. No files changed.\n'; }
run_install(){ preflight; prepare_candidates; show_changes; . "$TRANSACTION_LIBRARY"; dvsm_transaction_begin "$BACKUP_ROOT"; dvsm_backup_file "$MMDVM_BINARY"; dvsm_backup_file "$DASHBOARD_FUNCTIONS"; dvsm_backup_file "$DASHBOARD_STATUS"; dvsm_backup_file "$DVSWITCH_SCRIPT"; record_json "$P25_JSON"; record_json "$NXDN_JSON"; INSTALL_ACTIVE=1; SERVICE_RESTART_NEEDED=1; dvsm_install_candidate "$WORK_DIR/MMDVM_Bridge" "$MMDVM_BINARY"; cmp -s "$WORK_DIR/MMDVM_Bridge" "$MMDVM_BINARY"; dvsm_install_candidate "$WORK_DIR/functions" "$DASHBOARD_FUNCTIONS"; dvsm_install_candidate "$WORK_DIR/status" "$DASHBOARD_STATUS"; dvsm_install_candidate "$WORK_DIR/dvswitch" "$DVSWITCH_SCRIPT"; install_json "$WORK_DIR/P25Hosts.json" "$P25_JSON"; install_json "$WORK_DIR/NXDNHosts.json" "$NXDN_JSON"; php -l "$DASHBOARD_FUNCTIONS" >/dev/null; php -l "$DASHBOARD_STATUS" >/dev/null; bash -n "$DVSWITCH_SCRIPT"; validate_json "$P25_JSON" P25; validate_json "$NXDN_JSON" NXDN; systemctl try-restart "$MMDVM_SERVICE"; INSTALL_ACTIVE=0; SERVICE_RESTART_NEEDED=0; printf 'PASS: ordered MMDVM compatibility and P25/NXDN modules installed.\nBackup: %s\n' "$DVSM_TRANSACTION_DIR"; }
run_restore(){ local name=$1; preflight; [[ "$name" =~ ^install-[0-9]{8}-[0-9]{6}(-[0-9]+)?$ ]] || die "Invalid backup name: $name"; . "$TRANSACTION_LIBRARY"; dvsm_restore_backup_set "$BACKUP_ROOT/$name"; file -b "$MMDVM_BINARY" | grep -q ELF; php -l "$DASHBOARD_FUNCTIONS" >/dev/null; php -l "$DASHBOARD_STATUS" >/dev/null; bash -n "$DVSWITCH_SCRIPT"; systemctl try-restart "$MMDVM_SERVICE"; printf 'PASS: restore validation completed.\n'; }
