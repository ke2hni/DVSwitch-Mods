...
while IFS= read -r -d '' tracked_file; do
    relative_path="${tracked_file#"$REPO_ROOT"/}"
    ...
    if [[ -f "$tracked_file" ]]; then
        ...
        if grep -Iq . "$tracked_file"; then
            if grep -Eq -- '-----BEGIN (RSA |EC |OPENSSH |DSA )?PRIVATE KEY-----' "$tracked_file"; then
                report_failure "private key material detected: $relative_path"
            fi
        fi
    fi
done < <(git -C "$REPO_ROOT" ls-files -z | while IFS= read -r -d '' path; do printf '%s\0' "$REPO_ROOT/$path"; done)
