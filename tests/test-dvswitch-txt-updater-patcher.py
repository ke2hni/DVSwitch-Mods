...
with tempfile.TemporaryDirectory() as directory:
    ...
    json_extended = repaired.replace(
        '        downloadAndValidateDatabase "NXDNHosts.txt" "https://hostfiles.refcheck.radio/NXDNHosts.txt" NXDN\n',
        '        downloadAndValidateDatabase "NXDNHosts.txt" "https://hostfiles.refcheck.radio/NXDNHosts.txt" NXDN\n'
    ...
    require(
        module.patch_text(json_extended) == json_extended,
        "supported JSON extension was not accepted unchanged",
    )
    require_patch_error(
        json_extended.replace(expected_calls[1], "", 1),
        "JSON extension missing a required repaired TXT call",
    )
