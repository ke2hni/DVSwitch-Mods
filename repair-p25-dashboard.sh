...
# Repair stock P25 remote-command link detection in the locally installed
...
preflight() { require_root; for command in bash cmp cp date install mktemp mv php python3 sha256sum stat; do require_command "$command"; done; check_os; check_regular_file "$PATCHER"; check_regular_file "$TRANSACTION_LIBRARY"; check_regular_file "$TARGET"; php -l "$TARGET" >/dev/null; python3 "$PATCHER" --help >/dev/null; }
...
run_install() { preflight; prepare_candidate; show_result; if cmp -s "$TARGET" "$WORK_DIR/functions.php"; then printf 'PASS: P25 dashboard compatibility repair is already installed.\n'; return; fi; . "$TRANSACTION_LIBRARY"; dvsm_transaction_begin "$BACKUP_ROOT"; dvsm_backup_file "$TARGET"; INSTALL_ACTIVE=1; dvsm_install_candidate "$WORK_DIR/functions.php" "$TARGET"; cmp -s "$WORK_DIR/functions.php" "$TARGET"; php -l "$TARGET" >/dev/null; python3 "$PATCHER" --functions "$TARGET"; cmp -s "$WORK_DIR/functions.php" "$TARGET"; INSTALL_ACTIVE=0; printf 'PASS: P25 dashboard compatibility repair installed atomically.\nBackup: %s\n' "$DVSM_TRANSACTION_DIR"; }
run_restore() { local name=$1; preflight; [[ "$name" =~ ^install-[0-9]{8}-[0-9]{6}(-[0-9]+)?$ ]] || die "Invalid backup name: $name"; . "$TRANSACTION_LIBRARY"; dvsm_restore_backup_set "$BACKUP_ROOT/$name"; php -l "$TARGET" >/dev/null; printf 'PASS: functions.php restored from %s.\n' "$name"; }
