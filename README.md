...
title. For example, a host named `pi4test` displays `pi4test DVSwitch
Dashboard`. The hostname is generated by PHP when the page is rendered, so
the same installed modification displays the correct name on each node.

This is a surgical modification to the exact `<h2>DVSwitch Dashboard</h2>`
block in `index.php`. It does not modify dashboard layout, CSS, themes,
activity tables, or Local Activity logic.
