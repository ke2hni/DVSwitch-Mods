...
| `repair-ysf-dashboard-null.sh` | 1.0.2 | Corrects case-sensitive YSF room matching and prevents a literal `null` fallback | Completed and tested |
...
| `mod-p25-nxdn-friendly-names.sh` | 1.1.3 | Displays P25 and NXDN reflector names with sponsor and numeric fallbacks | Completed and tested |
| `mod-dstar-tx-ref.sh` | 1.0.3 | Adds D-Star Tx TG/Ref and reflector/module display | Completed and tested |
| `mod-dmr-friendly-names.sh` | 1.4.0 | Displays BrandMeister, TGIF, and STFU talkgroup names, blocks stale cross-mode talkgroups, wraps long names, and preserves DMR status across an empty UTC-day log | Completed and tested |
