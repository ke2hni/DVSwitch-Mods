...
=======
if marker in text or re.search(r'ceil\s*\(\s*\(float\)\s*\$listElem\[6\]\s*\)', text): raise SystemExit(2)
>>>>>>> 24879815a551e5bb90c32d677d5944f24a5efa05
...
=======
run_check() { preflight; set +e; prepare_candidate; status=$?; set -e; if [[ $status -eq 2 ]]; then printf 'ALREADY REPAIRED: dashboard duration handling is installed.\n'; return; fi; [[ $status -eq 0 ]] || exit "$status"; php -l "$WORK_DIR/functions.php" >/dev/null; printf 'READY: dashboard duration repair available; no files changed.\n'; }
run_install() { preflight; set +e; prepare_candidate; status=$?; set -e; if [[ $status -eq 2 ]]; then printf 'ALREADY REPAIRED: dashboard duration handling is installed.\n'; return; fi; [[ $status -eq 0 ]] || exit "$status"; php -l "$WORK_DIR/functions.php" >/dev/null; . "$TRANSACTION_LIBRARY"; dvsm_transaction_begin "$BACKUP_ROOT"; dvsm_backup_file "$TARGET"; INSTALL_ACTIVE=1; dvsm_install_candidate "$WORK_DIR/functions.php" "$TARGET"; php -l "$TARGET" >/dev/null; INSTALL_ACTIVE=0; printf 'PASS: dashboard duration repair installed. Backup: %s\n' "$DVSM_TRANSACTION_DIR"; }
>>>>>>> 24879815a551e5bb90c32d677d5944f24a5efa05
