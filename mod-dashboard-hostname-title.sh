...
if not all(old_count == 1 and new_count == 0 for old, old_count, new, new_count in states):
...
if action in ("--check", "check"):
    print("READY: exact original dashboard heading and browser-tab title targets found. No files changed."); raise SystemExit(0)
