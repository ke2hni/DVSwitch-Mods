...
# Repair the two malformed remote-command format strings in the locally
...
run_check() { preflight; prepare_candidate; show_result; printf 'PASS: supported MMDVM_Bridge pattern state. No files changed.\n'; }
run_dry_run() { run_check; }
run_install() { preflight; prepare_candidate; show_result; if cmp -s "$TARGET" "$WORK_DIR/MMDVM_Bridge"; then printf 'PASS: MMDVM spacing repairs are already installed.\n'; return; fi; . "$TRANSACTION_LIBRARY"; dvsm_transaction_begin "$BACKUP_ROOT"; dvsm_backup_file "$TARGET"; INSTALL_ACTIVE=1; dvsm_install_candidate "$WORK_DIR/MMDVM_Bridge" "$TARGET"; cmp -s "$WORK_DIR/MMDVM_Bridge" "$TARGET"; python3 "$PATCHER" --binary "$TARGET"; systemctl try-restart "$SERVICE"; systemctl is-active --quiet "$SERVICE"; INSTALL_ACTIVE=0; printf 'PASS: P25 and YSF MMDVM spacing repairs installed atomically.\nBackup: %s\n' "$DVSM_TRANSACTION_DIR"; }
