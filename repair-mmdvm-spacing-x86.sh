...
# Installer for explicitly recognized tight-layout x86 builds.
...
readonly BACKUP_ROOT="/var/backups/dvswitch-mods/mmdvm-spacing-x86"

WORK_DIR=""
INSTALL_ACTIVE=0

die() { printf 'ERROR: %s\n' "$1" >&2; exit 1; }
usage() { printf 'MMDVM x86 spacing repair %s (exact AMD64/i386 builds only)\nUsage: sudo %s {--check|--dry-run|--install|--restore BACKUP-NAME}\n' "$SCRIPT_VERSION" "$(basename "$0")"; }
...
run_install() { preflight; prepare_candidate; show_result; if cmp -s "$TARGET" "$WORK_DIR/MMDVM_Bridge"; then printf 'PASS: x86 MMDVM spacing repairs are already installed. No files changed.\n'; return; fi; . "$TRANSACTION_LIBRARY"; dvsm_transaction_begin "$BACKUP_ROOT"; dvsm_backup_file "$TARGET"; INSTALL_ACTIVE=1; dvsm_install_candidate "$WORK_DIR/MMDVM_Bridge" "$TARGET"; cmp -s "$WORK_DIR/MMDVM_Bridge" "$TARGET"; python3 "$PATCHER" --binary "$TARGET"; systemctl try-restart "$SERVICE"; systemctl is-active --quiet "$SERVICE"; INSTALL_ACTIVE=0; printf 'PASS: AMD64/i386 MMDVM spacing repairs installed atomically.\nBackup: %s\n' "$DVSM_TRANSACTION_DIR"; }
run_restore() { local name=$1; preflight; [[ "$name" =~ ^install-[0-9]{8}-[0-9]{6}(-[0-9]+)?$ ]] || die "Invalid backup name: $name"; . "$TRANSACTION_LIBRARY"; dvsm_restore_backup_set "$BACKUP_ROOT/$name"; file -b "$TARGET" | grep -Eq 'ELF (64-bit.*x86-64|32-bit.*Intel (80386|i386))'; systemctl try-restart "$SERVICE"; systemctl is-active --quiet "$SERVICE"; printf 'PASS: MMDVM_Bridge restored from %s.\n' "$name"; }
