...
def supported_base(value):
    if digest(value) in (supported_hash, os.environ["DVS_VIRGIN_STATUS_HASH"], os.environ["DVS_DMR_V7_STATUS_HASH"]):
        return True
    buttons_marker_count = sum(value.count(item) for item in buttons_markers)
    buttons_counts = (buttons_marker_count, value.count('function dvsButtonsDmrMasterHeading('), value.count('function dvsButtonsDmrMasterDisplay('))
...
verify_installed() {
    ...
    local dmr_v2_count dmr_v3_count dmr_v4_count dmr_v5_count dmr_v6_count dmr_v7_count buttons_marker_count
    ...
    buttons_marker_count=$(grep -Ec '// DVSwitch-Mode-Buttons: standalone DMR Master display v[1-6]$' "$TARGET" || true)
    ...
    if [[ $dvswitch_mods_marker_count -eq 1 && $buttons_marker_count -eq 0 ]]; then
        :
    elif [[ $dvswitch_mods_marker_count -eq 0 && $buttons_marker_count -eq 1 ]]; then
        :
    elif [[ $dmr_v2_count -eq 0 && $dmr_v3_count -eq 0 && $dmr_v4_count -eq 0 && $dmr_v5_count -eq 0 && $dmr_v6_count -eq 0 && $dmr_v7_count -eq 0 && $dmr_v8_count -eq 1 && $buttons_marker_count -eq 1 ]]; then
        :
    else
        return 1
    fi
