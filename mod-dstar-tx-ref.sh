...
patch_candidate() {
    ...
    DVS_DMR_V5_YSF_STATUS_HASH="$DMR_V5_YSF_STATUS_HASH" python3 - <<'PY_PATCH'
...
def require_downstream_markers(value, dmr_v1, dmr_v2, dmr_v3, dmr_v4, dmr_v5, ysf):
    counts = (
    ...
    if counts != (dmr_v1, dmr_v2, dmr_v3, dmr_v4, dmr_v5, ysf):
...
if markers == 0:
    ...
    if value == os.environ["DVS_DSTAR_STATUS_HASH"]:
        ...
        require_downstream_markers(text, 0, 0, 0, 0, 0, 0)
    elif value == os.environ["DVS_DMR_V1_STATUS_HASH"]:
        require_downstream_markers(text, 1, 0, 0, 0, 0, 0)
    elif value == os.environ["DVS_DMR_V2_STATUS_HASH"]:
        require_downstream_markers(text, 0, 1, 0, 0, 0, 0)
    elif value == os.environ["DVS_YSF_STATUS_HASH"]:
        require_downstream_markers(text, 0, 1, 0, 0, 0, 1)
    elif value == os.environ["DVS_DMR_V3_YSF_STATUS_HASH"]:
        require_downstream_markers(text, 0, 0, 1, 0, 0, 1)
    elif value == os.environ["DVS_DMR_V4_YSF_STATUS_HASH"]:
        require_downstream_markers(text, 0, 0, 0, 1, 0, 1)
    elif value == os.environ["DVS_DMR_V5_YSF_STATUS_HASH"]:
        require_downstream_markers(text, 0, 0, 0, 0, 1, 1)
    else:
